...
import { BADGE_HIERARCHY, BADGE_META, getPrimaryBadge, DEFAULT_GAMES, DEFAULT_RARITIES, DEFAULT_PAYMENT_METHODS, invalidateSiteConfig } from '@/lib/constants'
...
function SiteSettingsTab() {
  ...
  if (!config) return <EmptyState icon="⚙️" message="Could not load site config" />

  const games = config.games || DEFAULT_GAMES
  const rarities = config.rarities || DEFAULT_RARITIES
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#f9fafb' }}>🎮 Games</div>
          <button onClick={() => setGameModal('add')} style={S.actionBtn('#4ade80')}>+ Add Game</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {games.map(g => (
            <div key={g.id} style={{
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#f9fafb' }}>💳 Payment Methods</div>
          <button onClick={() => setPmModal('add')} style={S.actionBtn('#60a5fa')}>+ Add Method</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
  ...
  return (
    <Modal title={isEdit ? 'Edit Game' : 'Add Game'} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div>
          <label style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'block' }}>Display Name</label>
          <input value={form.label} onChange={e => set('label', e.target.value)} placeholder="e.g. Minecraft Brainrot" />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'block' }}>Emoji</label>
  ...
  return (
    <Modal title={isEdit ? 'Edit Rarity' : 'Add Rarity'} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div>
          <label style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'block' }}>Display Label</label>
          <input value={form.label} onChange={e => set('label', e.target.value)} placeholder="e.g. Divine" />
        </div>
        <div>
  ...
  return (
    <Modal title={isEdit ? 'Edit Payment Method' : 'Add Payment Method'} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div>
          <label style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'block' }}>Display Name</label>
          <input value={form.label} onChange={e => set('label', e.target.value)} placeholder="e.g. Zelle" />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'block' }}>Emoji</label>
          <input value={form.emoji} onChange={e => set('emoji', e.target.value)} placeholder="💳" maxLength={4} />
