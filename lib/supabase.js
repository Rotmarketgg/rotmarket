...
export async function getListings({ game, type, status, search, limit = 24, offset = 0 } = {}) {
  ...
  // Data query — no count: exact (avoids expensive full table COUNT)
  let dataQuery = supabase
    .from('listings')
    .select(`
      id, created_at, expires_at, user_id,
      title, game, rarity, price, type,
  ...
  // Lightweight count query — runs in parallel, much faster than count:exact on full select
