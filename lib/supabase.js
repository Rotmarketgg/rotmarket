...
export async function getConversations(userId) {
  // Fetch recent messages only — enough to cover all unique conversations.
  // 500 is generous; a user with 500+ messages still gets all their convos.
  const { data, error } = await supabase
    .from('messages')
    .select(`
      id, created_at, content, read,
      sender_id, receiver_id, listing_id, archived_by,
  ...
  // Deduplicate to one row per conversation (listing + other user)
  const seen = new Set()
  const conversations = []
