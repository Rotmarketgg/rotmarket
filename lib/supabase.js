...
export async function signIn(emailOrUsername, password) {
  ...
  if (!emailOrUsername.includes('@')) {
    // Username login: look up the profile first, then resolve the email.
    // Strategy 1: use the get_email_by_user_id RPC (requires the function to
    //             exist in Supabase with SECURITY DEFINER so it can read auth.users).
    // Strategy 2: fall back to signing in with a sentinel email format and letting
    //             Supabase return the real error — this never works, so we always
    //             need Strategy 1 or the user must supply their email.
    //
    // If get_email_by_user_id doesn't exist yet, create it in Supabase SQL editor:
    //
    //   create or replace function get_email_by_user_id(user_id uuid)
    //   returns text language plpgsql security definer as $$
    //   declare email text;
    //   begin
    //     select u.email into email from auth.users u where u.id = user_id;
    //     return email;
    //   end;
    //   $$;
    ...
    if (profileError || !profile) {
      throw new Error('No account found with that username. Try logging in with your email address instead.')
    }
    ...
    if (rpcError || !resolvedEmail) {
      // RPC missing or failed — give a clear actionable message
      throw new Error(
        'Username login is temporarily unavailable. Please log in with your email address.'
      )
    }
...
// getMessages — DEPRECATED. Use getMessagesPaginated instead.
// This wrapper exists only for any call sites that haven't been migrated yet.
// It delegates to getMessagesPaginated with a 200-message limit so long threads
// don't OOM the browser tab. Migrate call sites to getMessagesPaginated directly.
export async function getMessages(userId, otherUserId, listingId) {
  console.warn('[RotMarket] getMessages() is deprecated — use getMessagesPaginated()')
  const { messages } = await getMessagesPaginated(userId, otherUserId, listingId, null, 200)
  return messages
}
