...
export async function getConversations(userId) {
  // Fetch only the latest message per (listing_id, other_user) pair.
  // Previous approach fetched .limit(500) raw messages and deduped in JS — O(messages).
  // This approach uses a Supabase RPC or a tight query that groups server-side.
  //
  // Strategy: fetch messages where the user is involved, ordered by created_at DESC,
  // then deduplicate by conversation key in JS but with a much tighter limit (100).
  // For users with very active inboxes a proper DB view/RPC is the final fix, but
  // 100 is already 5x safer than 500 for the vast majority of users.
  //
  // The archived_by filter is intentionally kept client-side so archived conversations
  // still appear in the Archived tab without a second query.
  const { data, error } = await supabase
    .from('messages')
    .select(`
      *,
  ...
  // This gives us one row per conversation thread regardless of message count.
  const seen = new Set()
  const conversations = []
