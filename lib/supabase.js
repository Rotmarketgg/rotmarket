...
export async function signUp(email, password, username) {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      data: { username } // stored in auth metadata, used by trigger
    }
  })
  if (error) throw error

  // If email confirmation is disabled, user is immediately available
  // If enabled, the trigger will fire on confirmation and set username from metadata
  if (data.user) {
...
export async function updateListing(id, updates) {
  // Use safe_update_listing RPC to bypass RLS user_id filter issue
  const { data, error } = await supabase
    .from('listings')
    .update(updates)
    .eq('id', id)
