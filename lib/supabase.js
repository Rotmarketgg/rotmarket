...
// background, it does NOT release held Web Locks. On tab return, Supabase
// tries to re-acquire the lock, times out after 5000ms, and throws:
//   DOMException: The lock request is aborted
// This makes getSession() return null even when a valid token is sitting in
// localStorage — causing every page to appear logged out and data fetches
// to time out.
//
// Fix: replace the lock implementation with a no-op. We are a single-tab
// SPA with no concurrent writes; cross-tab lock serialization is unnecessary.
// The no-op is safe here because all auth operations are sequential.
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    lock: noopLock,                // bypass Web Locks — prevents tab-suspend deadlock
  },
})

// ─── TAB VISIBILITY RECOVERY ─────────────────────────────────────
// When a browser tab is suspended in the background, two things break:
//   1. The Supabase session lock (fixed above with noopLock)
//   2. The underlying HTTP/fetch connections — they go stale and hang on
//      resume until the OS re-establishes them (~300-1500ms depending on OS)
//
// Strategy:
//   a) Debounce the visibilitychange event — Firefox fires it 2-3x per tab
//      return due to focus/blur cycles. Without debounce, pages fire 2-3
//      parallel fetches that all time out.
//   b) Await getSession() so the token is confirmed fresh before pages fetch.
//   c) Add a 600ms delay before dispatching rotmarket:tab-visible — this gives
//      the OS network stack time to re-establish TCP connections after the tab
//      was suspended. Without this delay, the first fetch after wakeup often
//      hits a dead connection and hangs until withTimeout kills it.
if (typeof window !== 'undefined') {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    // Debounce: if visibilitychange fires multiple times in quick succession
    // (Firefox behaviour), only process the last one
    if (tabVisibleTimer) clearTimeout(tabVisibleTimer)

    tabVisibleTimer = setTimeout(async () => {
      tabVisibleTimer = null
      // Re-validate and refresh the session token. Must complete before
      // pages call getSessionUser() or they'll race against a stale token.
      await supabase.auth.getSession()
      // Reconnect realtime websocket killed during tab suspension
      supabase.realtime.connect()
      // Signal pages that the session is fresh and connections are ready
      window.dispatchEvent(new Event('rotmarket:tab-visible'))
    }, 600) // 600ms: enough for OS to re-establish TCP, short enough to feel instant
  })
}
...
// getSessionUser — returns the current user from the session.
// Waits up to 4s for auth to settle if the client is still initializing
// (e.g. on tab return after suspension, where INITIAL_SESSION hasn't fired yet).
// This prevents pages from seeing null and showing "not found" or redirecting
// to login during the ~200-800ms window where the Supabase client is reading
// the token from localStorage and validating it.
export async function getSessionUser() {
  const { data: { session } } = await supabase.auth.getSession()
  if (session?.user) return session.user

  // Session was null — could be genuinely logged out, OR the client is still
  // initializing (re-mounting after tab suspension). Wait up to 4s for
  // onAuthStateChange to fire with a real session before concluding logged-out.
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      unsub?.()
      resolve(null) // genuinely not logged in after 4s
    }, 4000)

    const { data: { subscription: unsub } } = supabase.auth.onAuthStateChange((event, sess) => {
      ...
      // Keep waiting on any other event (PASSWORD_RECOVERY, USER_UPDATED, etc.)
    })
  })
}
