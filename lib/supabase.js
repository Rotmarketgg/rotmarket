...
export async function getProfile(userId) {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      paypal_email, cashapp_handle, venmo_handle, revolut_handle,
...
function normalizePromotion(listing) {
  ...
  const eligible = badges.includes('VIP Max')
...
export async function getListings({ game, type, status, search, limit = 24, offset = 0, sort = 'promoted' } = {}) {
