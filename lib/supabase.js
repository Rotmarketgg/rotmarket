...
// may be stale or broken. Calling reconnect() forces Supabase to re-establish
// the connection immediately rather than waiting for the next query to fail.
if (typeof window !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      // Re-validate the auth session first so the token is fresh before any
      // page-level data fetches fire. Without this, auth checks race against
      // a waking network connection and can appear logged-out even though the
