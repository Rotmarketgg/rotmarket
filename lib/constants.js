...
// ─── RARITY STYLE LOOKUP ─────────────────────────────────────────
// Single source of truth — derived from RARITIES above.
// Keys map rarity id → { border, text, bg, glow } for use in UI components.
// utils.js getRarityStyle() reads from this map instead of duplicating values.
export const RARITY_STYLES = Object.fromEntries(
  // Both games share identical rarities — use fortnite as the canonical list
  RARITIES.fortnite.map(r => [r.id, {
    border: r.color,
    text:   r.color,   // components use border color for text tinting
    bg:     r.bg,
    glow:   r.glow,
  }])
)

export const BADGE_HIERARCHY = ['Owner', 'Moderator', 'VIP', 'Verified Trader']
