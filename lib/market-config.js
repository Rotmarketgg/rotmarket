...
const VERSION_KEY = 'rotmarket:market-config-version'
...
async function readConfigRows() {
  ...
  try {
    ...
    const hasDbRows = !gamesRes.error && !raritiesRes.error && !paymentsRes.error
    const games = hasDbRows && gamesRes.data?.length ? gamesRes.data : fallback.games
    const rarities = hasDbRows && raritiesRes.data?.length ? raritiesRes.data : fallback.rarities
    const payments = hasDbRows && paymentsRes.data?.length ? paymentsRes.data : fallback.payments

    // If client role cannot read catalog tables (missing GRANT/RLS), fallback to public API.
    if (!hasDbRows && typeof window !== 'undefined') {
      try {
        const res = await fetch('/api/market-config', { method: 'GET' })
        if (res.ok) {
          const json = await res.json()
          return {
            games: Array.isArray(json?.games) && json.games.length ? json.games : fallback.games,
            rarities: Array.isArray(json?.rarities) && json.rarities.length ? json.rarities : fallback.rarities,
            payments: Array.isArray(json?.paymentMethods) && json.paymentMethods.length ? json.paymentMethods : fallback.payments,
          }
        }
      } catch {}
    }
...
export function bumpMarketConfigVersion() {
  clearMarketConfigCache()
  if (typeof window === 'undefined') return
  const stamp = String(Date.now())
  try { localStorage.setItem(VERSION_KEY, stamp) } catch {}
  window.dispatchEvent(new Event('rotmarket:market-config-updated'))
}
...
export function useMarketConfig() {
  ...
  useEffect(() => {
    ...
    const onUpdated = () => refreshNow()
    window.addEventListener('storage', onStorage)
    window.addEventListener('rotmarket:market-config-updated', onUpdated)
    return () => {
      window.removeEventListener('storage', onStorage)
      window.removeEventListener('rotmarket:market-config-updated', onUpdated)
    }
  }, [])
